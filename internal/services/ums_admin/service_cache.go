...
func (s *umsAdminCacheService) DelAdmin(ctx core.Context, adminId int64) {
...
func (s *umsAdminCacheService) DelResourceList(ctx core.Context, adminId int64) {
	key := fmt.Sprintf("%s:%s:%d", REDIS_DATABASE, REDIS_KEY_RESOURCE_LIST, adminId)
	s.cache.Del(key)
}

func (s *umsAdminCacheService) DelResourceListByRole(ctx core.Context, roleId int64) {

}

func (s *umsAdminCacheService) DelResourceListByRoleIds(ctx core.Context, roleIds []int64) {

}

func (s *umsAdminCacheService) DelResourceListByResource(ctx core.Context, resourceId int64) {

}

func (s *umsAdminCacheService) GetAdmin(ctx core.Context, username string) *ums_admin.UmsAdmin {
...
func (s *umsAdminCacheService) SetAdmin(ctx core.Context, admin *ums_admin.UmsAdmin) {
	key := REDIS_DATABASE + ":" + REDIS_KEY_ADMIN + ":" + admin.Username
	adminBytes, _ := json.Marshal(admin)
	s.cache.Set(key, string(adminBytes), REDIS_EXPIRE)
}

func (s *umsAdminCacheService) GetResourceList(ctx core.Context, adminId int64) []*ums_resource.UmsResource {
	key := fmt.Sprintf("%s:%s:%d", REDIS_DATABASE, REDIS_KEY_RESOURCE_LIST, adminId)
	ret, err := s.cache.Get(key)
	if err != nil {
		return nil
...
func (s *umsAdminCacheService) SetResourceList(ctx core.Context,
