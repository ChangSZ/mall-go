...
type listRequest struct {
	BrandName       string `form:"brandName" binding:"omitempty"`
...
type listResponse struct {
	...
	List      []dto.SmsHomeBrand `json:"list"`
}
...
func (h *handler) List(ctx *gin.Context) {
	...
	list, total, err := h.smsHomeBrandService.List(
		ctx, req.BrandName, req.RecommendStatus, req.PageSize, req.PageNum)
	if err != nil {
		log.WithTrace(ctx).Error(err)
